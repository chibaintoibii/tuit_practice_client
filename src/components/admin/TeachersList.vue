<template>
    <div class="container-fluid">
        <div class="row mt-3">
            <div class="col-3">
                <div class="input-group">
                    <input type="search" class="form-control" :placeholder="this.$localeToText('search') + '...'" />
                    <button type="button" class="btn btn-custom-primary">
                        <i class="fas fa-search"></i>
                    </button>
                </div>
            </div>
            <div class="col-9 d-flex justify-content-end">
                <button class="btn btn-custom-primary me-3"><i class="fa-solid fa-user-plus"></i>  {{ this.$localeToText('add_new_teacher') }}</button>
                <button class="btn btn-success"><i class="fa-solid fa-download"></i> {{ this.$localeToText('download_to_excel') }}</button>

            </div>
        </div>
        <div class="row my-4">
            <div class="col-12">
                <table class="custom-table">
                    <thead>
                        <tr>
                            <th scope="col">№</th>
                            <th scope="col">{{ this.$localeToText('photo') }}</th>
                            <th scope="col">
                                <span style="cursor: pointer;" @click="sortBy(0)">{{ this.$localeToText('fullname') }}
                                    <i class="fa-solid ms-1" :class="[sortIcons[sortingFields[0] + 1]]"></i>
                                </span>
                            </th>
                            <th scope="col">
                                <span style="cursor: pointer;" @click="sortBy(1)">
                                    Username <i class="fa-solid ms-1" :class="[sortIcons[sortingFields[1] + 1]]"></i>
                                </span>
                            </th>
                            <th scope="col">
                                <span style="cursor: pointer;" @click="sortBy(2)">
                                    {{ this.$localeToText('count_of_groups') }} <i class="fa-solid ms-1"
                                        :class="[sortIcons[sortingFields[2] + 1]]"></i>
                                </span>
                            </th>
                            <th scope="col">
                                <span style="cursor: pointer;" @click="sortBy(3)">
                                    {{ this.$localeToText('phone_num') }} <i class="fa-solid ms-1"
                                        :class="[sortIcons[sortingFields[3] + 1]]"></i>
                                </span>
                            </th>
                            <th scope="col">{{ this.$localeToText('actions') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="o in 10">
                            <td>{{ o }}</td>
                            <td class="d-flex justify-content-center align-items-center">
                                <div class="img">
                                    <img src="data:image/jpeg;base64,"
                                        alt="">
                                </div>
                            </td>
                            <td>Xamidjonov Islom Ilhomovich</td>
                            <td style="letter-spacing: 1px;">
                                <router-link :to="`/admin/teachers/${'chibaintoibii'}`"
                                    class="text-decoration-underline text-custom-dark">chibaintoibii</router-link>
                            </td>
                            <td>4</td>
                            <td>+998 (91) 443-19-21</td>
                            <td>
                                <button class="btn btn-custom-primary me-1 px-2 py-1"><i
                                        class="fa-regular fa-pen-to-square"></i></button>

                                <button class="btn btn-danger px-2 py-1"><i class="fa-regular fa-trash-can"></i></button>
                            </td>
                        </tr>
                    </tbody>
                </table>
                <pagination></pagination>
            </div>
        </div>
    </div>
</template>
<script lang="ts">
import Pagination from '../Pagination.vue';
export default {
    name: 'AdminTeachers',
    mounted() {

    },
    components: {
        Pagination,
    },
    data() {
        return {
            currentPage: 1,
            itemsPerPage: 10, // Number of items per page
            data: [], // Your paginated data,
            sortIcons: ['fa-sort-down', 'fa-sort', 'fa-sort-up'],
            sortingFields: [0, 0, 0, 0],
        };
    },
    computed: {
        totalPages() {
            return Math.ceil(100 / this.itemsPerPage);
        },
        paginatedData() {
            const start = (this.currentPage - 1) * this.itemsPerPage;
            const end = start + this.itemsPerPage;
            return this.data.slice(start, end);
        },
    },
    methods: {
        changePage(newPage: number) {
            this.currentPage = newPage;
        },
        sortBy(n: number) {
            for (let i = 0; i < 4; i++) {
                if (i == n) continue;
                this.sortingFields[i] = 0;
            }
            if (this.sortingFields[n] === 0) {
                this.sortingFields[n] = 1;
            } else if (this.sortingFields[n] === 1) {
                this.sortingFields[n] = -1;
            } else {
                this.sortingFields[n] = 0;
            }
        }
    },
}
</script>
<style scoped>
table td .img {
    width: 30px;
}

table td .img img {
    width: 100%;
    height: 100%;
    object-fit: cover;
    background-position: center center;
}
</style>